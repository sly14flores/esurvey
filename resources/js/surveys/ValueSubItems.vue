<template>
	<div>
		<b-modal size="lg" :id="'sub-item-'+section+item+value" title="Sub Items" ok-only @ok="ok" @hidden="reset">
						...
						<tbody name="list" is="transition-group">
							<tr v-for="(sub_item, vsi) in sub_items" :key="sub_item.index">
								...
								<td>
									...
									<b-modal :id="'confirm-sub-item-delete-'+section+item+value+vsi" title="Confirmation" @ok="deleteSubItem(sub_item)">
	...
	export default {
		...
		methods: {
			...
			subItems() {
				...
				this.$nextTick(() => {
					this.$bvModal.hide('sub-item-'+this.section+this.item+this.value)
			...
			deleteSubItem(sub_item) {
				...
				axios.delete('api/survey/section/item/value/sub_item/'+sub_item.id, this.$store.state.config).then(response => {
