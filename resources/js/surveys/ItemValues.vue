		...
		<div class="table-reponsive">
			<transition name="fade">
				<table class="table" v-if="has_values">
					<thead>
						<tr><th v-if="has_display && sectionItem.items[item].text_is_multiple">Required?</th><th v-if="has_display">Display</th><th v-if="has_display">Translation (Optional)</th><th v-if="has_value">Value</th><th v-if="has_highest_lowest">Highest</th><th v-if="has_highest_lowest">Lowest</th><th v-if="has_infographic">Image</th><th v-if="has_min">Min</th><th v-if="has_max">Max</th><th v-if="has_data_type">Data Type</th><th v-if="has_row_type">Row Type</th><th v-if="has_sub">Sub Items</th><th></th></tr>
					</thead>
					<tbody name="list" is="transition-group">
						<tr v-for="(value, vi) in values" :key="value.index">
							<td v-if="has_display && sectionItem.items[item].text_is_multiple">
							...
							<td v-if="has_display">
								<div class="form-group">
									<input type="text" class="form-control" v-model.trim="$v.values.$each[vi].display.$model" :class="{'is-invalid': $v.values.$each[vi].display.$error}" :disabled="oldSurvey && !onEdit">
							...
							<td v-if="has_value">
								<div class="form-group">
									<input type="text" class="form-control" v-model.trim="$v.values.$each[vi].siv_value.$model" :class="{'is-invalid': $v.values.$each[vi].siv_value.$error}" :disabled="oldSurvey && !onEdit">
							...
							<td v-if="has_infographic">
								<div class="images-container">
									<div v-if="value.siv_infographic != null" class="image-container">
										<div class="controls">
											<a href="javascript:;" class="control-btn remove" @click="removeSivIg(section,item,vi)">
												<i class="ion-trash-a"></i>
											</a>
										</div>														
										<img :id="'siv_'+section+item+vi" class="image siv-infographic" :src="value.siv_infographic">
									</div>
									<input type="file" style="display: none;" :id="'upload-siv-infographic_'+section+item+vi" @change="sivIg(section,item,vi)">
									<a v-if="value.siv_infographic == null" href="javascript:;" @click="addSivIg(section,item,vi)">
							...
							<td v-if="has_min">					
								<div class="form-group">
									<input type="text" class="form-control" v-model.trim="$v.values.$each[vi].siv_min.$model" :class="{'is-invalid': $v.values.$each[vi].siv_min.$error}" :disabled="oldSurvey && !onEdit">
							...
							<td v-if="has_max">
								<div class="form-group">
									<input type="text" class="form-control" v-model.trim="$v.values.$each[vi].siv_max.$model" :class="{'is-invalid': $v.values.$each[vi].siv_max.$error}" :disabled="oldSurvey && !onEdit">
							...
							<td>
								...
								<b-modal :id="'confirm-value-delete-'+section+item+vi" title="Confirmation" @ok="deleteValue(value)">
...
<script>
	...
	import ValueSubItems from './ValueSubItems'
	...
	export default {
		...
		computed: {
			...
			values: {
				...
				set(value) {
					this.$store.commit('values',{section: this.section, item: this.item, values: value})				
				}
			},
			...
			has_display() {
				...
				return this.sectionItem.items[this.item].item_type != 3 || 
					(this.sectionItem.items[this.item].item_type == 3 && this.sectionItem.items[this.item].text_is_multiple)
			...
			has_value() {
				...
				return this.sectionItem.items[this.item].item_type == 2 ||
						this.sectionItem.items[this.item].item_type == 4 ||
						this.sectionItem.items[this.item].item_type == 5 ||
						this.sectionItem.items[this.item].item_type == 6						
			...
			has_sub() {
				...
				return this.sectionItem.items[this.item].item_type == 1 ||
						...
						this.sectionItem.items[this.item].item_type == 3 ||
						this.sectionItem.items[this.item].item_type == 4 ||
						this.sectionItem.items[this.item].item_type == 5 ||			
						this.sectionItem.items[this.item].item_type == 7						
			...
			has_infographic() {
				...
				return this.sectionItem.items[this.item].item_type == 6 &&
						this.sectionItem.items[this.item].item_presentation == 2						
		...
		methods: {
			...
			deleteValue(value) {
				...
				axios.delete('api/survey/section/item/value/'+value.id, this.$store.state.config).then(response => {
			...
			sivIg(s,i,vi) {
				...
				let file = ($('#upload-siv-infographic_'+s+i+vi)[0].files)[0];
